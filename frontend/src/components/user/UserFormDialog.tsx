...
const UserFormDialog: React.FC<UserFormDialogProps> = ({ 
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (!canPerformAction) {
      setError(t('access_denied_action')); // ✅ Fixed: replaced hardcoded text
      return;
    }

    if (!formData.username.trim()) {
      setError(t('username_required')); // ✅ Fixed: replaced hardcoded text
      return;
    }

    if (!user && !formData.password.trim()) {
      setError(t('password_required')); // ✅ Fixed: replaced hardcoded text
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || t('failed_to_save_user')); // ✅ Fixed: replaced hardcoded text
      }
  ...
  if (!canPerformAction) {
    return (
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle>{t('access_denied')}</DialogTitle> {/* ✅ Fixed: replaced hardcoded text */}
        <DialogContent>
          <Typography color="error">
            {t('access_denied_user', { action: user ? 'edit' : 'create' })} {/* ✅ Fixed: replaced hardcoded text */}
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={onClose}>{t('close')}</Button> {/* ✅ Fixed: replaced hardcoded text */}
        </DialogActions>
      </Dialog>
    );
  }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <form onSubmit={handleSubmit}>
        <DialogTitle>
          {user ? t('edit_user') : t('create_new_user')} {/* ✅ Fixed: replaced hardcoded text */}
        </DialogTitle>
        <DialogContent>
          {error && (
          ...
          <TextField
            margin="dense"
            label={user ? t('new_password_placeholder') : t('password')} 
            fullWidth
            variant="outlined"
            type="password"
          ...
          {canGetRole && availableRoles.length > 0 && (
            <FormControl component="fieldset" sx={{ mt: 2 }}>
              <FormLabel component="legend">{t('roles')}</FormLabel> {/* ✅ Fixed: replaced hardcoded text */}
              <FormGroup>
        ...
        <DialogActions>
          <Button onClick={onClose} disabled={loading}>
            {t('cancel')} {/* ✅ Fixed: replaced hardcoded text */}
          </Button>
          <Button 
            type="submit" 
            variant="contained" 
            disabled={loading}
          >
            {loading ? <CircularProgress size={20} /> : (user ? t('update_user') : t('create_user'))} {/* ✅ Fixed: replaced hardcoded text */}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
